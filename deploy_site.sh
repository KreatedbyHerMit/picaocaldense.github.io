...
git add .
git commit -m "Initial site deploy" || echo "Nothing to commit"

# --- Create GitHub repository if it doesn't exist ---
if ! gh repo view "$GITHUB_USER/$REPO_NAME" >/dev/null 2>&1; then
    gh repo create "$REPO_NAME" --public --source=. --remote=origin --push || { echo "Repo creation failed"; exit 1; }
else
...
echo "✅ Deployment script completed. Visit https://$CUSTOM_DOMAIN"
